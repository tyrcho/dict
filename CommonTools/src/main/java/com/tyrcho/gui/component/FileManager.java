...
import com.tyrcho.gui.component.FileActionListener;
...
public class FileManager {
	...
	public FileManager(Component parent, String extension,
	...
	private boolean saveAsClicked() {
		...
		if (JFileChooser.APPROVE_OPTION == fileChooser.showSaveDialog(parent)) {
			...
			if (!file.exists()
				...
				if (saved) {
					setCurrentFile(file);
					JOptionPane.showMessageDialog(parent, "Le fichier "
							+ currentFile + " a été enregistré.");
					setModified(false);
				}
				return saved;
			}
	...
	private boolean saveClicked() {
		...
		if (saved) {
			JOptionPane.showMessageDialog(parent, "Le fichier " + currentFile
					+ " a été enregistré.");
			setModified(false);
		}
		return saved;
